...
class Course {
    ...
    public function addCourse($course_name, $course_code, $description, $credit_hours, $department_id, $teacher_id) {
        $sql = "INSERT INTO courses (title, course_code, credit_units, department_id, teacher_id, semester_id) 
                VALUES (?, ?, ?, ?, ?, 1)"; // Using semester_id = 1 as default, adjust as needed
    
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param("ssiii", $course_name, $course_code, $credit_hours, $department_id, $teacher_id);
