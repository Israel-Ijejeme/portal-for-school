...
class Grade {
    ...
    // Get recent grades by teacher
    public function getRecentGradesByTeacher($teacher_id, $limit = 5) {
        $query = "SELECT g.*, 
                  c.course_code, 
                  u.full_name as student_name,
                  COALESCE(
                      CASE
                          WHEN g.score >= 70 THEN 'A'
                          WHEN g.score >= 60 THEN 'B'
                          WHEN g.score >= 50 THEN 'C'
                          WHEN g.score >= 45 THEN 'D'
                          WHEN g.score >= 40 THEN 'E'
                          ELSE 'F'
                      END
                  ) as grade
                  FROM grades g
